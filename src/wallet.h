...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 XIM at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zXIM send
enum ZerocoinSpendStatus {
    ZXIM_SPEND_OKAY = 0,                            // No error
    ZXIM_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZXIM_WALLET_LOCKED = 2,                         // Wallet was locked
    ZXIM_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZXIM_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZXIM_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZXIM_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZXIM_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZXIM_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZXIM_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZXIM_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZXIM_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZXIM_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZXIM_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZXIM_SPENT_USED_ZXIM = 14,                      // Coin has already been spend
    ZXIM_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZXIM_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZXimBlockBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZXIMOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzXIMWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzXIMTracker> zpivTracker;
    ...
    void setZWallet(CzXIMWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzXIMTracker>(new CzXIMTracker(strWalletFile));
    }

    CzXIMWallet* getZWallet() { return zwalletMain; }
    ...
    void setZXimBlockAutoBackups(bool fEnabled)
    {
    ...
    /** zXIM reset */
    boost::signals2::signal<void()> NotifyzXIMReset;
