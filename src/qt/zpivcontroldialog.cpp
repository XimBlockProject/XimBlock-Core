...
std::set<std::string> ZXimBlockControlDialog::setSelectedMints;
std::set<CMintMeta> ZXimBlockControlDialog::setMints;

ZXimBlockControlDialog::ZXimBlockControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZXimBlockControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZXimBlockControlDialog::~ZXimBlockControlDialog()
{
    delete ui;
}

void ZXimBlockControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXimBlockControlDialog::updateList()
{
...
void ZXimBlockControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXimBlockControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZXimBlock_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXimBlockControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZXimBlockControlDialog::GetSelectedMints()
{
...
void ZXimBlockControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
