...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::XIM);
    ...
    if (!settings.contains("nAnonymizeXimBlockAmount"))
        settings.setValue("nAnonymizeXimBlockAmount", 1000);

    nAnonymizeXimBlockAmount = settings.value("nAnonymizeXimBlockAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeXimBlockAmount"))
        SoftSetArg("-anonymizeximblockamount", settings.value("nAnonymizeXimBlockAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ximblock.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeXimBlockAmount:
            return QVariant(nAnonymizeXimBlockAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeXimBlockAmount:
            nAnonymizeXimBlockAmount = value.toInt();
            settings.setValue("nAnonymizeXimBlockAmount", nAnonymizeXimBlockAmount);
            emit anonymizeXimBlockAmountChanged(nAnonymizeXimBlockAmount);
            break;
        case CoinControlFeatures:
