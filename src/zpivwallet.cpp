...
CzXIMWallet::CzXIMWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXIMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXIMSeed_deprecated()) {
                    LogPrintf("%s: Updated zXIM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzXIMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXIMCount(nCountLastUsed);
    else if (!walletdb.ReadZXIMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXIMWallet::Lock()
{
    seedMaster = 0;
}

void CzXIMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXIMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZXIM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzXIMWallet::LoadMintPoolFromDB()
{
...
void CzXIMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXIMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXIMWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzXIMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXIM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzXIMWallet::SeedToZXIM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzXIMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXIMWallet::GenerateDeterministicZXIM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXIMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXIMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
