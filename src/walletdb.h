...
public:
    ...
    bool WriteZXIMSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZXIMSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZXIMSeed_deprecated(uint256& seed);
    bool EraseZXIMSeed();
    bool EraseZXIMSeed_deprecated();

    bool WriteZXIMCount(const uint32_t& nCount);
    bool ReadZXIMCount(uint32_t& nCount);
